...
- `src/linkedin/services/diagnostics.py` — the one check list behind `automation doctor` and `automation status`. `health` was a second copy under different names and is gone; CI's smoke step calls `automation doctor --json --time 09:00`.
