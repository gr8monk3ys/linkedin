...
def test_import_search_results_skips_duplicates(contact_repo):
    results = [{"name": "Alice Smith", "headline": "ML Engineer at Stripe", "linkedin_url": "https://linkedin.com/in/alice"}]
...
def test_imported_contacts_are_seeded_with_a_follow_up_date(contact_repo):
    ...
    added, _ = import_search_results([{"name": "Alice", "headline": "ML Engineer at Acme", "linkedin_url": "u/1"}], contact_repo)
    assert added[0]["follow_up_date"] is not None


def test_scraped_profile_creates_contact(contact_repo):
    data = {"name": "Jane Doe", "headline": "Senior ML Engineer at OpenAI", "location": "SF", "about": "Building AI systems."}
...
def test_scraped_profile_updates_existing(contact_repo):
    contact_repo.add({
        "id": 1, "name": "Jane Doe", "title": "ML Engineer", "company": "Old Co",
        "linkedin_url": "https://linkedin.com/in/janedoe", "status": "connected", "activities": [],
    })
    contact = import_scraped_profile({"name": "Jane Doe", "headline": "Senior ML Engineer at OpenAI"}, "https://linkedin.com/in/janedoe", contact_repo)
    assert contact["company"] == "OpenAI"
    assert len(contact_repo.list_all()) == 1
