...
def automate_post(text, draft_id, calendar_id, dry_run, headless):
    ...
    _publish(text, draft_id=draft_id, calendar_id=calendar_id if calendar_entry is not None else None, dry_run=dry_run, headless=headless)
...
def posts_review(publish_on):
    ...
    for draft in pending:
        console.print(Panel(draft["content"], title=f"Candidate #{draft['id']} — {draft.get('topic', '')}", border_style="cyan"))
        choice = click.prompt("  [a]pprove / [r]eject / [s]kip", type=click.Choice(["a", "r", "s"]), default="s", show_choices=False)
        if choice == "a":
