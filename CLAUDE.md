...
- `src/linkedin/cli/` — one module per command group. `_common.py` holds the root `cli` group, the `console`, and the lazy `_app` handle; `__init__.py` imports every group module for registration and re-exports what tests need. Click groups + Rich formatting only; all calls go to services. Tests patch a name where the code looks it up, not where it was defined: fleet facts on the group module that imported it (`linkedin.cli.posts.collect_fleet_facts`), crontab I/O on `linkedin.scheduling.install`, which the CLI calls through rather than importing.
...
- `src/linkedin/app.py` — `App(data_dir)`: every repo and service for one directory. `cli/_common.py` holds a lazy `_app` handle built from the environment on first use, so importing the CLI never touches disk. Nothing may compute a Click option default from `_app` either: doing so builds the App at import and bakes in whatever data dir was set then. Commands reach `_app.contact_svc`, `_app.data_dir.recaps`, and so on.
...
- **The home feed and profile pages were rebuilt with obfuscated markup (verified 2026-09-03).** No `feed-shared-update-v2`, no `data-urn`, no `h1` name. `get_feed_posts` reads cards by shape through `FEED_POSTS_SCRIPT` (the smallest ancestor of a like button that also holds a Comment button; the button patterns are passed in from `LIKE_BUTTON`/`COMMENT_BUTTON`, never copied into the JS) and tags each with `FEED_CARD_TAG=<index>`; `like_post`/`comment_on_post` match that tag value exactly through `_feed_card_at`, because tag order is button order, not DOM order. A script that throws is a `feed_card` miss, not a quiet feed. `scrape_profile` scrolls first on both paths (sections lazy-load; unscrolled, the only "About" line on the page is the footer's link list) and falls back to `_profile_from_text`, whose shape constants (`PROFILE_PRONOUNS_OR_DEGREE`, `PROFILE_SECTION_HEADINGS`, `PROFILE_FOOTER_LINES`, `PROFILE_NOT_A_LOCATION`) live in `selectors.py` like every other selector. Known gap: a collapsed About stops at the "…see more" fold. Recent-activity pages still carry the old markup, which is why `react(profile_url)` never needed this. The like control's label is now `Reaction button state: <state>`; anything but `no reaction` means we already reacted, and `LIKE_ALREADY_REACTED` must keep matching it or `engage` will un-like posts.
...
- **The top card's buttons mount after the card does.** Waiting for the section is not enough: a read in between finds a card with nothing in it, and the code falls through to the More menu. That is how a profile showing a plain Follow button reported "no Follow in the More menu" for all five targets. `_wait_for_top_card_actions` waits for an actual control, and both the invitation and follow paths call it.
- **Most of the ranked queue cannot be invited, only followed.** `automate follow` and `automate follow-due` exist for that: on 2026-09-09 an invitation sweep skipped 21 of 23 as follow-only, and following reached all five of the top-ranked people it could not. `follow_profile` insists the Follow it clicks names the profile's own person, because a post in someone's activity carries a Follow for *its* author.
- **Nothing reaches past the Session for a page.** The page object is `_page`, private. Everything a caller needs is a verb, so budget, pacing and dry run cannot be skipped by going around them. `engage_feed` read the feed straight off the page and `automate easy-apply` resumed a wizard the same way; both paid nothing and neither had a test, because `FakeSession` handed them a MagicMock.
- **The test double honours scope, and that is load-bearing.** A lookup scoped to a locator in `tests/fake_page.py` sees that element's own `children` and nothing registered on the page. While it fell back to the page registry, a scoped and an unscoped lookup were indistinguishable, so reverting any of the scoping fixes kept the suite green. `register_top_card(...)` puts controls inside the card; a dialog carries its Send button as a child.
- **A scheduled run does not send unless asked.** `ScheduleSpec.send_connections` defaults to `False` and `automation schedule` needs `--send-connections`. It cannot default on: `automation doctor --fix` builds a spec from defaults, so repairing a missing schedule would be how unattended outbound sending got switched on. `doctor --fix` states `send_connections=False` explicitly rather than inheriting it, so the guarantee survives someone changing the default.
- **A click is not evidence.** `send_connection_request` confirms the invitation by the dialog closing, and only a dialog still offering Send counts as still open, so an unrelated modal cannot make a real send read as unconfirmed. Every caller that advances a contact goes through `automation_service.invitation_confirmed`, because `session._write` flattens `degraded` to `ActionResult("ok", ...)` and truthiness alone means "we clicked Send". `automate connect` skipped that check and reported a send the page had refused to confirm. The budget is spent before the flattening on purpose: an invitation we cannot vouch for must be assumed sent, or the next run sends a second one. Returning `ok` on the click alone reported "Sent invitation to Jonathan Shin" before there was anything to show for it.
